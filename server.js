#!/usr/bin/env node
/**
 * Gequhai Music Search - Proxy Server
 * Serves static files and proxies API requests to NeteaseCloudMusicApi (port 3000)
 */
...
proxy.on('error', (err, req, res) => {
  ...
  if (!res.headersSent) {
    res.status(502).json({
      code: 502,
      msg: 'API proxy error: ' + err.message,
      data: null
    });
  }
});
...
app.use(express.static(STATIC_DIR, {
  ...
  setHeaders: (res, filePath) => {
    // Cache static assets
    if (filePath.match(/\.(js|css|png|jpg|gif|ico|svg)$/)) {
...
// Proxy to Meting search API
app.get('/api/meting-search', (req, res) => {
    const keyword = req.query.keyword || '';
    const limit = req.query.limit || '30';
    const targetUrl = `https://met.api.xiaoguan.fit/api?server=netease&type=search&id=${encodeURIComponent(keyword)}`;
    
    try {
        // Use curl to fetch (bypasses Node.js HTTPS proxy issues)
        const raw = execSync(`curl -s --max-time 10 "${targetUrl}"`, { encoding: 'utf-8' });
        // Parse the JSON array and simplify
        const songs = JSON.parse(raw);
        const simplified = songs.slice(0, parseInt(limit)).map((s, i) => {
            // Extract song ID from the url field
            const urlMatch = s.url && s.url.match(/id=(\d+)/);
        ...
        res.json({ code: 200, data: simplified, count: songs.length });
    } catch(e) {
        console.error('[Meting API Error]', e.message);
        res.json({ code: 502, msg: 'API unavailable', data: [] });
    }
});

// Proxy to Paugram song API
app.get('/api/song-detail', (req, res) => {
    const songId = req.query.id || '';
    const targetUrl = `https://api.paugram.com/netease/?id=${songId}`;
    ...
    try {
        ...
        console.error('[Paugram API Error]', e.message);
        res.json({ code: 502, msg: 'API unavailable' });
    }
});

// API Proxy: /api/* -> http://localhost:3000/*
app.use('/api', (req, res) => {
  // Remove /api prefix and forward to target
...
<footer class="bg-dark py-1 my-0 border-top mt-4">
    <div class="container" style="max-width: 900px;">
        <div class="mt-1 d-flex flex-wrap justify-content-center align-items-center">
            <a href="/" class="text-white text-center">GeKeCy_Music &copy; 2026</a>
...
function generateSearchPage(keyword, count, rowsHtml) {
...
// Handle form submission: /s/?keyword=xxx -> redirect to /s/xxx
app.get('/s/', (req, res) => {
    const keyword = req.query.keyword;
    if (keyword) {
...
app.get('/s/:keyword', (req, res) => {
    ...
    try {
        ...
        const html = generateSearchPage(keyword, songs.length, rowsHtml);
        res.send(html);
    } catch (e) {
        console.error('[SSR Search Error]', e.message);
        res.send(generateSearchPage(keyword, 0, '<tr><td colspan="3" style="text-align:center;color:#999;padding:20px;">搜索失败，请稍后再试</td></tr>'));
    }
});

// Server-rendered play page
app.get('/play/:id', (req, res) => {
    ...
    try {
        ...
        console.error('[SSR Play Error]', e.message);
        res.send(generatePlayPage(null, songId));
    }
});
...
app.listen(PORT, () => {
  console.log(`Gequhai server running @ http://localhost:${PORT}`);
